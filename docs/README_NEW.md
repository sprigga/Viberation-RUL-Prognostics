...
現代化的 Web 應用程式，整合 Vue 3 前端與 FastAPI 後端，用於 CPC 線性滑軌的振動信號分析與故障診斷。
...
### 滑軌規格
- `POST /api/guide-specs` - 新增規格
- `GET /api/guide-specs` - 獲取所有規格
- `GET /api/guide-specs/{id}` - 獲取特定規格
...
- `GET /api/health-trend/{guide_spec_id}` - 獲取健康趨勢
...
### 1. 新增滑軌規格
進入「滑軌規格」頁面，新增您要監測的線性滑軌參數。

### 2. 計算理論頻率
使用「頻率計算」工具，根據滑軌參數和運行速度計算 BPF、BSF 等理論頻率。
...
### 預壓狀態評估
系統會根據 CPC 預壓等級（VC/V0/V1/V2）自動評估預壓狀態：
- RMS 過低 → 預壓失效（鬆動）
- Kurtosis 過高 → 預壓不足（間隙）
...
### 自訂滑軌型號
在 `backend/analysis.py` 的 `_get_guide_parameters` 方法中新增參數。
...
- [Vibration Analysis Application](Vibration%20Analysis%20Application%20of%20CPC%20Linear%20Guides.md) - 應用方案
